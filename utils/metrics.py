...
class Evaluator():
    def __init__(self, video_loader, txt_loader):
        # 注意这里将 img_loader 改为 video_loader
        self.video_loader = video_loader  # gallery：视频集合
        self.txt_loader = txt_loader  # query：文本集合
    ...
    def _compute_embedding(self, model):
        ...
        # 视频部分：调用 encode_video 而非 encode_image
        for pid, video in self.video_loader:
            video = video.to(device)
            with torch.no_grad():
                video_feat = model.encode_video(video).cpu()
            gids.append(pid.view(-1))
            gfeats.append(video_feat)
    ...
    def _compute_embedding_tse(self, model):
        ...
        # 视频部分，改用 encode_video_tse，并注意变量名称
        for pid, video in self.video_loader:
            video = video.to(device)
            with torch.no_grad():
                video_feat = model.encode_video_tse(video).cpu()
            gids.append(pid.view(-1))
            gfeats.append(video_feat)
        gids = torch.cat(gids, 0)
        gfeats = torch.cat(gfeats, 0)
    ...
    def eval(self, model, i2t_metric=False):
        ...
        gfeats = F.normalize(gfeats, p=2, dim=1)  # 视频特征
